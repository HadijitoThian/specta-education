...
const EXAMPLES: Ex[] = [
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // EXPANSION ROUND — lifts Economics coverage from 31 → ~50 questions.
  // ═══════════════════════════════════════════════════════════════════════════

  { topicCode: "E1.1", marks: 2,
    question: "Define scarcity.",
    markScheme: "1 mark: **resources** (or wants) are **limited** / finite.\n1 mark: but **human wants are unlimited** (or 'greater than the resources available').\nFull-mark answer: 'Scarcity is the economic problem that human wants are unlimited but the resources to satisfy them are limited, forcing choices to be made.'",
    source: "exam-style" },

  { topicCode: "E1.4", marks: 4,
    question: "Using a Production Possibility Curve (PPC), explain how a country can experience ECONOMIC GROWTH. Use a diagram description in your answer.",
    markScheme: "Mark for each clear point, max 4:\n• **Diagram (1):** Draw a standard PPC with two goods on the axes (e.g. consumer goods on y-axis, capital goods on x-axis). Show the curve bowed outward.\n• **Original PPC** represents max output combinations given current resources + technology.\n• **OUTWARD SHIFT of the entire PPC** = economic growth. **(B1)**\n• **Causes** of the shift (any one earns the mark): more resources (e.g. discovery of natural resources, immigration, capital investment), better technology, improved education + training (human capital), better institutions.\n• A point INSIDE the curve = unemployment / inefficiency (resources not fully used); economic growth must move the WHOLE FRONTIER out, not just shift along it.\nKey distinction: moving along the curve = reallocating, not growth. Shifting outward = growth.",
    source: "exam-style" },

  { topicCode: "E2.4", marks: 4,
    question: "Explain TWO factors (other than the good's own price) that could shift the supply curve for coffee to the LEFT.",
    markScheme: "1 mark per factor + 1 mark for explanation (max 4):\n• **Rise in costs of production** (e.g. fertiliser, labour, fuel for transport) → coffee farmers can produce less profitably at every price → supply ↓.\n• **Bad weather / poor harvest** (drought, frost, pest outbreak) → coffee beans damaged → less supply available at every price.\n• **Higher taxes on producers** (e.g. export tax on coffee in producing country) → producers receive less per unit → supply ↓.\n• **Cut to subsidies** (if previously the government subsidised coffee farming) → costs effectively rise → supply ↓.\n• **Number of producers FALLS** (coffee farmers exit the industry — e.g. switching to a more profitable crop) → total supply falls.\nTwo distinct factors, each clearly explained in the context of coffee = 4 marks.",
    source: "exam-style" },

  { topicCode: "E2.5", marks: 4,
    question: "Using a demand-and-supply diagram, analyse the effect on the price and quantity of bicycles if the government imposes a tax on the makers of bicycles.",
    markScheme: "Mark for each correct point + diagram description (max 4):\n• **Tax on producers raises their costs** → at every price they're willing to supply LESS.\n• **Supply curve shifts UPWARD/LEFTWARD** (S → S₁).\n• **Diagram: original equilibrium at P*, Q*; new equilibrium at P₁, Q₁ where P₁ > P* and Q₁ < Q*.**\n• Result: **equilibrium price RISES + equilibrium quantity FALLS.**\n• **Consumers and producers share the burden** of the tax — how much each bears depends on price elasticities (PED inelastic → consumers bear more; PED elastic → producers absorb more).\nDiagram is essential — describe axes, curves, intersection points.",
    source: "exam-style" },

  { topicCode: "E2.8", marks: 4,
    question: "Explain TWO factors that affect the price elasticity of SUPPLY (PES) of a product.",
    markScheme: "1 mark per factor + 1 mark for explanation (max 4):\n• **Time period** — in the short run producers can't easily increase output (capacity is fixed); supply is INELASTIC. In the long run they can build new factories, hire more workers, etc. → supply more ELASTIC.\n• **Spare capacity** — if producers have unused machines / idle workers, they can quickly increase output when price rises → ELASTIC. If at full capacity → INELASTIC.\n• **Stocks** — if firms hold large stocks they can release more onto the market quickly → ELASTIC. Goods that can't be stored (fresh fish, electricity) → INELASTIC.\n• **Ease of switching production** — firms that can quickly switch lines (e.g. car manufacturer between models) have ELASTIC supply.\n• **Mobility of factors of production** — if labour + capital can quickly move into the industry → ELASTIC.\nTwo factors, each explained = 4 marks.",
    source: "exam-style" },

  { topicCode: "E3.5", marks: 4,
    question: "Compare a SOLE TRADER with a PUBLIC LIMITED COMPANY (plc). Refer to ownership, liability, and finance.",
    markScheme: "Mark for each clear comparison point (max 4):\n• **Ownership**: Sole trader has ONE owner; plc has many shareholders (often thousands or millions).\n• **Liability**: Sole trader has **UNLIMITED** liability — personally responsible for ALL business debts (personal assets at risk). plc has **LIMITED** liability — shareholders only risk their investment.\n• **Finance**: Sole trader relies on personal savings + small loans → LIMITED finance. plc can issue shares on the stock exchange → access to LARGE amounts of capital from many investors.\n• **Control**: Sole trader has full control (no other owners to consult). In a plc, ownership is separate from control — directors run the business on behalf of shareholders.\n• **Privacy + regulation**: Sole trader's accounts are private. plc must publish detailed annual accounts (transparent but more regulation).\nAny clear contrast covering the 3 axes asked = full marks.",
    source: "exam-style" },

  { topicCode: "E3.8", marks: 6,
    question: "Analyse the disadvantages of a MONOPOLY for CONSUMERS and the possible benefits for the FIRM holding the monopoly.",
    markScheme: "L3 (5–6): At least two clear consumer disadvantages + two firm benefits + brief comment on the trade-off.\nL2 (3–4): One side covered well.\nL1 (1–2): Listing only.\n\n**Consumer DISADVANTAGES:**\n• **Higher prices** — with no competition, the monopolist can charge above the competitive price → consumer surplus falls.\n• **Less choice** — only one producer → narrower product range.\n• **Lower quality / less innovation** — no competitive pressure to improve.\n• **Worse service** — no risk of customers switching elsewhere.\n• **Possible exploitation** — captive consumers (utilities, transport) have no alternative.\n\n**FIRM benefits:**\n• **Supernormal (abnormal) profit** — long-run profits above the normal level.\n• **Economies of scale** — large size can mean lower unit costs.\n• **Funds for R&D** — supernormal profits can be reinvested in research (drug companies argue this for patents).\n• **Natural monopoly efficiency** — for utilities (water, rail track), one big network is more efficient than many.\n\n**Trade-off / regulation:**\n• Governments often regulate monopolies (price caps, quality standards, breakup powers) to capture the efficiency benefits while protecting consumers from exploitation.\n\nL3 answer covers both sides AND the trade-off / regulatory response.",
    source: "exam-style" },

  { topicCode: "E4.3", marks: 4,
    question: "Explain TWO costs of rapid economic growth that a country might experience.",
    markScheme: "1 mark for naming + 1 mark for explanation (max 4):\n• **Environmental damage** — more factories + transport → air pollution, water pollution, deforestation, loss of biodiversity, climate change.\n• **Depletion of non-renewable resources** — fossil fuels, minerals consumed faster than they can be replaced.\n• **Increased inequality** — gains often concentrated in cities + skilled workers, leaving rural areas behind.\n• **Inflation** — if growth is demand-led + economy is near full capacity, demand-pull inflation rises.\n• **Loss of culture / community** — rapid urbanisation breaks family + village networks.\n• **Stress + health problems** — longer hours, less leisure, mental health issues from competitive society.\nAny two with clear explanations of mechanism.",
    source: "exam-style" },

  { topicCode: "E4.4", marks: 4,
    question: "Identify TWO types of unemployment and give a specific example of EACH.",
    markScheme: "1 mark for type + 1 mark for valid example (max 4):\n• **Frictional** — short-term, between jobs (just left old job, looking for new). Example: a graduate looking for their first job for 2 months after university.\n• **Seasonal** — work only available at certain times of year. Example: ski-resort staff in summer, beach-resort staff in winter, agricultural workers between harvests.\n• **Structural** — long-term mismatch between worker skills and jobs available, often due to industry decline. Example: UK coal miners after the 1980s pit closures; manufacturing workers replaced by automation in 2010s–2020s.\n• **Cyclical** — caused by a fall in aggregate demand during a recession. Example: many workers laid off across all sectors during the 2008 financial crisis / 2020 COVID lockdown.\n• **Technological** — replaced by machines or software. Example: telephone-exchange operators replaced by automated switching; bank tellers by ATMs.\nAny two distinct types with a specific named example = full marks.",
    source: "exam-style" },

  { topicCode: "E4.5", marks: 4,
    question: "Distinguish between DEMAND-PULL inflation and COST-PUSH inflation. Give the typical cause of each.",
    markScheme: "**Demand-pull (2 marks):** inflation caused by **TOO MUCH DEMAND** chasing too few goods. Typical when the economy is at or near full capacity and aggregate demand keeps rising. Causes: consumer spending boom, government spending boom, large tax cuts, rising exports, lower interest rates.\n**Cost-push (2 marks):** inflation caused by **RISING COSTS OF PRODUCTION** — firms pass higher costs on as higher prices. Causes: rising wages (especially with strong unions), rising raw material prices (oil, food, metals), depreciating currency raising import costs, new indirect taxes.\nKey distinction: demand-pull starts on the DEMAND side; cost-push starts on the SUPPLY side. Policy responses differ — demand-pull responds well to contractionary monetary policy; cost-push doesn't (rate rises may worsen growth without curing the underlying cost shock).",
    source: "exam-style" },

  { topicCode: "E5.1", marks: 4,
    question: "Explain TWO limitations of using GDP per capita as a measure of a country's standard of living.",
    markScheme: "1 mark for naming + 1 mark for explanation (max 4):\n• **Doesn't show distribution** — GDP per capita is an AVERAGE; it hides inequality. A country could have rising GDP per capita while most people get poorer.\n• **Excludes non-market activity** — subsistence farming, household work, childcare, the informal economy are all unrecorded → developing countries especially understated.\n• **Doesn't capture non-material wellbeing** — leisure time, freedom, health, education quality, environment, security all matter for living standards but aren't in GDP.\n• **Ignores externalities** — pollution caused by GDP-generating activity reduces wellbeing but doesn't reduce GDP (in fact, cleanup also adds to GDP — paradox).\n• **Doesn't measure happiness or life satisfaction.**\n• **Currency comparison issues** — converting GDP to USD uses exchange rates that may not reflect actual cost of living (PPP adjustments help).\nAny two distinct limitations with clear explanations = full marks.",
    source: "exam-style" },

  { topicCode: "E5.2", marks: 4,
    question: "Distinguish between ABSOLUTE poverty and RELATIVE poverty, giving a typical measurement of each.",
    markScheme: "**Absolute poverty (2 marks):** income below a level needed to afford basic necessities (food, shelter, clothing, water). Measured against a fixed threshold. The World Bank uses **US$2.15 per day** (in 2017 PPP prices) as the international extreme-poverty line. Same threshold applied across countries.\n**Relative poverty (2 marks):** income LOW RELATIVE TO OTHERS in the same country/society. Measured as a percentage of median income (e.g. below 60% of national median = relative poverty in UK, EU). Even a 'middle-income' person in a rich country can be in relative poverty if they fall far behind their neighbours.\nKey distinction: absolute is about survival; relative is about social inclusion + inequality. Absolute poverty has fallen globally over decades; relative poverty depends on inequality, which has risen in many countries.",
    source: "exam-style" },

  { topicCode: "E6.1", marks: 4,
    question: "Explain TWO benefits to a country of specialising in producing goods that it has an ADVANTAGE in producing, and then trading.",
    markScheme: "1 mark for naming + 1 mark for explanation (max 4):\n• **Higher world output / efficiency** — when each country produces what it does best (lowest opportunity cost), TOTAL world output is higher than if every country tried to produce everything.\n• **Access to a wider range of goods** — consumers can buy products the country can't (or shouldn't) produce itself (e.g. Indonesia gets cold-climate fruits via imports; Switzerland gets tropical fruits).\n• **Lower prices** — specialisation + economies of scale + competition reduce unit costs → cheaper goods for consumers.\n• **Faster technology + skills transfer** — international trade exposes domestic firms to global best practices.\n• **Larger markets** — domestic firms aren't limited to selling at home; can sell to the world → growth potential.\nAny two well-developed = full marks.",
    source: "exam-style" },

  { topicCode: "E6.4", marks: 4,
    question: "Identify the FOUR main components of the CURRENT ACCOUNT of the balance of payments.",
    markScheme: "All four required for B4 (B1 each):\n• **Trade in goods (visibles)** — exports of goods minus imports of goods.\n• **Trade in services (invisibles)** — exports minus imports of services (tourism, banking, shipping, insurance, IT services).\n• **Primary income** — net income from foreign investments (interest, profits, dividends received from abroad minus those paid abroad) + employee compensation across borders.\n• **Secondary income (transfers)** — government aid, foreign-worker remittances home, charitable donations across borders — net transfers received minus paid.\nA current-account SURPLUS means total receipts > total payments; DEFICIT is the opposite.\nB1 if 2-3 named; B0 if just 1.",
    source: "exam-style" },

  { topicCode: "E2.11", marks: 4,
    question: "Identify TWO roles the public (government) sector plays in a MIXED economic system, and give one example of each.",
    markScheme: "1 mark for role + 1 mark for example (max 4):\n• **Provision of public goods** — non-rival, non-excludable goods the private sector won't provide. Example: national defence, street lighting, public parks.\n• **Provision of merit goods** — goods under-consumed if left to the market (positive externalities ignored). Example: free public education, public healthcare, vaccinations.\n• **Regulation** — set + enforce rules. Example: monopoly regulation, banking regulation, food safety, environmental standards.\n• **Redistribution of income** — progressive taxation + welfare benefits to reduce inequality. Example: unemployment benefits, pensions, child allowance.\n• **Correcting externalities** — taxes on goods with negative externalities (cigarette tax, carbon tax) or subsidies for positive ones (solar panels).\n• **Macroeconomic stability** — central bank policy, fiscal policy, employment programmes.\nAny two distinct roles with clear examples.",
    source: "exam-style" },

  { topicCode: "E1.2", marks: 2,
    question: "Define 'capital' as a factor of production.",
    markScheme: "Full-mark definition: **CAPITAL is any man-made aid to production** (1) — physical goods used to produce other goods. Examples: machinery, factories, tools, computers, vehicles, buildings. (1)\nKey examiner trap: students often confuse 'capital' with 'money'. **Money is NOT a factor of production** — it's a medium of exchange that's used to BUY capital. Capital itself is the physical equipment.",
    source: "exam-style" },

  { topicCode: "E3.2", marks: 4,
    question: "Explain TWO factors that would cause a household to SAVE more of its disposable income.",
    markScheme: "1 mark per factor + 1 mark for explanation (max 4):\n• **Higher interest rates** — banks pay more on deposits → saving becomes more attractive (higher reward for delayed consumption).\n• **Pessimism / uncertainty about the future** — fear of job loss, recession, illness → 'precautionary' saving rises.\n• **Falling consumer confidence** — same mechanism; risk-aversion rises.\n• **Higher disposable income** — past a certain level, additional income tends to be saved rather than spent (lower marginal propensity to consume at high incomes).\n• **Saving for a specific large purchase** (house deposit, car, education, wedding).\n• **Tax-advantaged saving schemes** (e.g. tax-free ISAs in UK, retirement accounts) make saving more attractive.\n• **Demographic** — middle-aged adults save more than young + old (lifecycle hypothesis).\nAny two distinct factors with explanation.",
    source: "exam-style" },

  { topicCode: "E4.2", marks: 6,
    question: "Analyse how the central bank can use INTEREST RATES to control inflation. Refer to the transmission mechanism.",
    markScheme: "L3 (5–6): Clear chain showing how interest-rate changes flow through to inflation, with named intermediate steps + at least one limitation.\nL2 (3–4): Some of the chain explained, others missing.\nL1 (1–2): States the policy but no mechanism.\n\nKey chain (build each link):\n• To fight inflation, the central bank **RAISES the base interest rate**.\n• Commercial banks raise their lending + borrowing rates in line.\n• **Borrowing becomes more expensive** → households + firms borrow less.\n• **Consumer spending falls** — mortgages cost more (less disposable income), big-ticket items (cars, furniture) bought on credit fall in demand.\n• **Investment by firms falls** — new projects funded by loans become less profitable.\n• **Saving becomes more attractive** → households save more, spend less.\n• **Currency tends to APPRECIATE** as foreign capital flows in seeking higher returns → imports cheaper → imported-inflation pressure falls.\n• **Net effect: aggregate demand FALLS → demand-pull inflation eases.**\n\n**Limitations:**\n• Time lags (12–18 months for full effect).\n• Less effective against cost-push inflation (rate rises don't reduce oil prices).\n• High rates can tip the economy into recession.\n• Heavily indebted households may default if rates rise sharply.\n\nL3 answer covers the full chain AND a caveat.",
    source: "exam-style" },
];
