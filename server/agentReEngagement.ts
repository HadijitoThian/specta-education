    ...
    for (const lead of coldLeads) {
      ...
      try {
        ...
        try {
          const llmResponse = await invokeLLM({
            messages: [
              ...
              { role: "user", content: `Write a re-engagement email for ${lead.name} who showed interest ${lead.daysSinceLastContact} days ago (source: ${lead.source}, interest: ${lead.interest || "study abroad"}). Theme: ${template.hook}. End with link to https://www.spectaeducation.com/contact or WhatsApp https://wa.me/628182183880` }
            ],
          });
          const content = llmResponse.choices?.[0]?.message?.content;
          emailBody = typeof content === "string" ? content : "";
        } catch {
          emailBody = `Hai ${lead.name}! Kami dari SpecTa Education ingin mengingatkan bahwa konsultasi GRATIS masih tersedia untuk kamu. Hubungi kami di WhatsApp: +62 818 218 388`;
        }
        ...
        const emailHtml = `<div style="font-family:'Segoe UI',sans-serif;max-width:600px;margin:0 auto;">
          ...
          <div style="text-align:center;"><a href="https://wa.me/628182183880" style="display:inline-block;background:#25d366;color:#fff;text-decoration:none;padding:10px 24px;border-radius:8px;font-weight:600;">💬 WhatsApp Kami</a></div></div>
