...
export const users = mysqlTable("users", {
  ...
  // Per-counselor public intake link / QR token (students self-register here).
  intakeToken: varchar("intakeToken", { length: 32 }),
...
/**
 * Password reset tokens for email+password auth. The plain token only lives
 * in the email link; the DB stores a SHA-256 hash so a DB leak doesn't allow
 * password resets.
 */
...
export const igcseTopics = mysqlTable("igcse_topics", {
  id: int("id").autoincrement().primaryKey(),
  subject: mysqlEnum("subject", ["math", "physics", "economics", "business"]).default("math").notNull(),
